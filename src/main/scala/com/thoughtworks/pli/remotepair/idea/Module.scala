...
trait Module extends UtilsModule {
  ...
  lazy val tabEventHandler = new TabEventHandler(getFileByRelative, invokeLater, publishEvent, closeFile, getOpenFileDescriptor, publishSyncFilesRequest, logger)
  ...
  lazy val handleResetTabRequest = new HandleResetTabRequest(getSelectedTextEditor, getEditorPath, invokeLater, publishEvent)
  lazy val isCaretSharing = new IsCaretSharing(getProjectInfoData)
  lazy val moveCaret = new MoveCaret(invokeLater, getTextEditorsOfPath, isCaretSharing)
