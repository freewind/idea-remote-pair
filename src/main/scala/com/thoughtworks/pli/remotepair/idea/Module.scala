...
trait Module extends UtilsModule {
  ...
  lazy val connectedClient = new ConnectedClient(currentProject, isSubPath, createFileTree)
  lazy val logger = new PluginLogger(ideaLogger, connectedClient)
  ...
  lazy val startServer = new StartServer(currentProject, myPlatform, getLocalIp, serverPortInGlobalStorage, logger, connectedClient, showMessageDialog, showErrorDialog: ShowErrorDialog)
  lazy val createFileTree = new CreateFileTree(fileTreeNodeDataFactory)
  lazy val isInPathList = new IsInPathList(currentProject)
  lazy val publishSyncFilesRequest = new PublishSyncFilesRequest(connectedClient)
  ...
  lazy val publishCreateDocumentEvent = new PublishCreateDocumentEvent(connectedClient)
  lazy val clientVersionedDocumentFactory: ClientVersionedDocument.Factory = new ClientVersionedDocument(_)(logger, connectedClient, newUuid, getCurrentTimeMillis)
  ...
  lazy val handleChangeContentConfirmation = new HandleChangeContentConfirmation(currentProject, connectedClient, ideaPlatform, logger, clientVersionedDocuments, writeToProjectFile, highlightNewContent, synchronized)
  ...
  lazy val handleMoveCaretEvent = new HandleMoveCaretEvent(currentProject, ideaPlatform, connectedClient)
  lazy val handleCreateServerDocumentRequest = new HandleCreateServerDocumentRequest(currentProject, ideaPlatform, connectedClient)
  lazy val getDocumentLength = new GetDocumentLength(getDocumentContent)
  lazy val highlightPairSelection = new HighlightPairSelection(currentProject, ideaPlatform, connectedClient, logger)
  lazy val handleSyncFilesRequest = new HandleSyncFilesRequest(connectedClient)
  lazy val handleMasterWatchingFiles = new HandleMasterWatchingFiles(connectedClient, ideaPlatform, logger)
  ...
  lazy val handleGetPairableFilesFromPair = new HandleGetWatchingFilesFromPair(connectedClient)
  lazy val handleServerStatusResponse = new HandleServerStatusResponse(connectedClient)
  ...
  lazy val handleClientInfoResponse = new HandleClientInfoResponse(connectedClient)
  ...
  lazy val handleWatchFilesChangedEvent = new HandleWatchFilesChangedEvent(connectedClient, syncFilesForSlaveDialogFactory)
  lazy val handleEvent = new HandleEvent(handleOpenTabEvent, handleCloseTabEvent, myPlatform, connectedClient, handleChangeContentConfirmation, handleMoveCaretEvent, highlightPairSelection, handleSyncFilesRequest, handleMasterWatchingFiles, handleCreateServerDocumentRequest, handleCreateDocumentConfirmation, handleGetPairableFilesFromPair, handleJoinedToProjectEvent, handleCreatedProjectEvent, handleServerStatusResponse, handleClientInfoResponse, handleSyncFilesForAll, handleSyncFileEvent, handleCreateDirEvent, handleDeleteFileEvent, handleDeleteDirEvent, handleCreateFileEvent, handleRenameDirEvent, handleRenameFileEvent, handleMoveDirEvent, handleMoveFileEvent, handleDocumentSnapshotEvent, handleWatchFilesChangedEvent, logger, md5: Md5)
  lazy val connectionFactory: Connection.Factory = (channelHandlerContext) => new Connection(channelHandlerContext)(logger)
  lazy val myChannelHandlerFactory: MyChannelHandler.Factory = () => new MyChannelHandler(connectedClient, handleEvent, pairEventListeners, connectionFactory: Connection.Factory, logger: PluginLogger)
  ...
  lazy val watchFilesDialogFactory: WatchFilesDialog.Factory = (extraOnCloseHandler) => new WatchFilesDialog(extraOnCloseHandler)(myPlatform, connectedClient, pairEventListeners, isSubPath, getSelectedFromFileTree, getListItems, removeSelectedItemsFromList, removeDuplicatePaths, initListItems, initFileTree, getProjectWindow, showErrorDialog, isInPathList: IsInPathList)
  ...
  lazy val connectServerDialogFactory: ConnectServerDialog.Factory = () => new ConnectServerDialog(myPlatform, pairEventListeners, myChannelHandlerFactory: MyChannelHandler.Factory, clientFactory: Client.Factory, serverHostInProjectStorage, serverPortInProjectStorage, clientNameInCreationInProjectStorage, clientNameInJoinInProjectStorage, getProjectWindow, newUuid, watchFilesDialogFactory: WatchFilesDialog.Factory, copyProjectUrlDialogFactory: CopyProjectUrlDialog.Factory, projectUrlInProjectStorage, syncFilesForSlaveDialogFactory: SyncFilesForSlaveDialog.Factory, connectedClient: ConnectedClient)
  lazy val getCaretOffset = new GetCaretOffset
  lazy val handleIdeaFileEvent = new HandleIdeaFileEvent(currentProject, myPlatform, connectedClient, logger, clientVersionedDocuments, writeToProjectFile: WriteToProjectFile)
  lazy val handleCaretChangeEvent = new HandleCaretChangeEvent(connectedClient, logger, getDocumentContent, getCaretOffset: GetCaretOffset)
  lazy val handleDocumentChangeEvent = new HandleDocumentChangeEvent(myPlatform, connectedClient, publishCreateDocumentEvent, newUuid, logger, clientVersionedDocuments: ClientVersionedDocuments)
  lazy val handleSelectionEvent = new HandleSelectionEvent(connectedClient, logger, getSelectionEventInfo)
  lazy val handleFileTabEvents = new HandleFileTabEvents(publishCreateDocumentEvent, logger, connectedClient, tabEventsLocksInProject)
  ...
  lazy val myVirtualFileAdapterFactory: MyVirtualFileAdapter.Factory = () => new MyVirtualFileAdapter(currentProject, handleIdeaEvent, myPlatform, connectedClient, logger, clientVersionedDocuments, writeToProjectFile, isSubPath, ideaFactories: IdeaFactories)
  lazy val syncFilesForSlaveDialogFactory: SyncFilesForSlaveDialog.Factory = () => new SyncFilesForSlaveDialog(connectedClient, watchFilesDialogFactory, ideaPlatform, pairEventListeners, getProjectWindow)
  lazy val syncFilesForMasterDialogFactory: SyncFilesForMasterDialog.Factory = () => new SyncFilesForMasterDialog(ideaPlatform, connectedClient, watchFilesDialogFactory, pairEventListeners, getProjectWindow)
  lazy val copyProjectUrlToClipboard = new CopyProjectUrlToClipboard(projectUrlInProjectStorage, copyToClipboard)
  lazy val statusWidgetPopups = new StatusWidgetPopups(connectedClient, ideaPlatform, localIp, syncFilesForMasterDialogFactory, syncFilesForSlaveDialogFactory, showErrorDialog, copyProjectUrlToClipboard)
  lazy val createMessageConnection = new CreateMessageConnection(getMessageBus, currentProject)
  lazy val pairStatusWidgetFactory: PairStatusWidget.Factory = () => new PairStatusWidget(statusWidgetPopups, logger, connectedClient, createMessageConnection)
